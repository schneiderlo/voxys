...
## Water Visual Match Plan

Goal: move the current river rendering closer to `water.jpeg`.

### Current Gap

- [x] Water height is acceptable at `-230.0`, but visual style is not close enough.
- [x] Current water is too solid teal.
- [x] Reference water has warmer sky reflection: peach, gold, and pale blue.
- [x] Current shore edge has too much cyan rim.
- [x] Reference water is glossier and softer.
- [x] Current reflections are approximate and too weak.
- [x] Overall scene lacks the warm atmospheric haze seen in the reference.

### Implementation Steps

1. **Capture Baselines**
   - [x] Keep current screenshot as baseline.
   - [x] Capture 2-3 fixed camera angles.
   - [x] Compare against `water.jpeg` after every tuning pass.

2. **Tune Water Color**
   - [x] Keep `water.height = -230.0`.
   - [x] Make shallow water less cyan and more pale blue-green.
   - [x] Make deep water darker blue-teal.
   - [x] Add a warm reflected-sky tint.
   - [x] Reduce pure saturated teal in mid-depth water.

3. **Improve Reflections**
   - [x] Increase reflection strength.
   - [x] Bias grazing-angle water toward sky reflection.
   - [x] Soften screen-space terrain reflections.
   - [x] Avoid sharp reflection bands and noise.

4. **Improve Shore Blending**
   - [x] Reduce bright cyan rim at banks.
   - [x] Widen shore fade.
   - [x] Lower shallow bed tint strength.
   - [x] Add subtle sand/rock bleed instead of glow.

5. **Improve Depth Feel**
   - [x] Increase absorption in deeper parts.
   - [x] Keep transparency mostly in shallow water.
   - [x] Slightly darken tight canyon water.
   - [x] Avoid making the whole river opaque.

6. **Add Warm Atmosphere**
   - [x] Add warm distance haze over water.
   - [x] Blend water toward peach/pale-blue sky at distance.
   - [x] Keep terrain haze consistent with water haze.

7. **Validate**
   - [x] Run:
     ```bash
     nix-shell --run 'bazel test //tests:voxy_tests --test_output=errors'
     ```
   - [x] Rebuild native.
   - [x] Capture a screenshot.
   - [x] Compare against `water.jpeg`.

8. **WASM Verification**
   - [x] Use a temporary local `third_party/emsdk` directory to satisfy the tracked broken symlink for verification.
   - [x] Run:
     ```bash
     nix-shell --run 'bazel build --config=wasm //:voxy_wasm'
     ```
   - [x] Run browser version.
   - [x] Confirm no WebGPU bind group crash.

9. **Commit**
   - [x] Commit source, config, and test changes only.
   - [x] Do not commit screenshots unless requested.

### Water Artifact Follow-Up

- [x] Remove terrain texture bed tint from water shading so vertical canyon walls do not show through the water.
- [x] Reduce terrain screen-space reflection influence in shallow water.
- [x] Add `MIN_WATER_DEPTH` gate so very shallow sloped terrain is not rendered as water.
- [x] Clamp shallow-water reflection and specular lift to avoid white/milky water sheets.
- [x] Re-run native tests after the artifact fix.
- [x] Capture final artifact-fix screenshot.
- [x] Re-tune reflection and color after the artifact fix; current water is structurally cleaner and warmer.
- [x] Add screen-space shoreline proximity mask so banks get a controlled warm/foam transition without sampling terrain wall color through the water.
- [x] Add broad flow/depth variation and a small always-present sky wash so top-down water no longer reads as a flat solid teal fill.
- [x] Add displaced water-surface intersection in the ray caster so the water is no longer a mathematically flat plane.
- [x] Add heightfield-aware shoreline depth clamping so shallow banks are driven by nearby terrain height, not only screen-space proximity.
- [x] Do final non-content art tuning for warmer water haze, stronger bank transition, and water normals matched to the displaced surface.
- [x] Capture shoreline-mask screenshots:
  - `screenshots/water_visual_match/shoreline_mask/view_0.png`
  - `screenshots/water_visual_match/shoreline_mask/view_1.png`
