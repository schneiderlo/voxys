...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if not math.isfinite(args.meters_per_sample) or args.meters_per_sample <= 0.0:
        raise SystemExit("Error: --meters-per-sample must be positive.")
    if args.voxy_cell_scale is not None and (
        not math.isfinite(args.voxy_cell_scale) or args.voxy_cell_scale <= 0.0
    ):
        raise SystemExit("Error: --voxy-cell-scale must be positive.")
    if args.min_meters is not None and not math.isfinite(args.min_meters):
        raise SystemExit("Error: --min-meters must be finite.")
    if args.max_meters is not None and not math.isfinite(args.max_meters):
        raise SystemExit("Error: --max-meters must be finite.")
...
def write_metadata(
    ...
    metadata = {
        ...
        "voxy": {
            "heightmap": str(ldh_path) if ldh_path is not None else None,
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        metadata_ldh_path = None if args.no_compress else output_path
        write_metadata(
            metadata_path,
            args,
            terrain,
        ...
        if args.no_compress:
            print("No LDH file was created; compress the raw heightmap before using it in voxy.cfg.")
        else:
            print("Suggested voxy.cfg values:")
            print(f"  terrain.heightmap = \"{output_path}\"")
            print(f"  terrain.height_scale = {encoded.height_scale:.3f}")
            cell_scale = (
                args.voxy_cell_scale
                if args.voxy_cell_scale is not None
                else args.meters_per_sample / float(args.scale)
            )
