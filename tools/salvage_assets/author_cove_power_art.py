"""Original molded Cove engine/propeller presentations; background export, no images.
...
PARTS = ('engine', 'propeller')
VISUAL_IDS = {'engine': 600, 'propeller': 700}
PALETTE = toy.PALETTE


def engine(lod, part):
    m = art.Model('engine', lod)
    # Retain the exact cowling, bracket well, lower leg and shaft endpoints.
    m.box('engine_cover', (0, .12, 1), (.5, .36, .5), 'cream', .055)
    mount = m.box('transom_bracket', (0, -.4, .24), (.5, .08, .74), 'steel', .018)
    m.mounts(part, [mount])
    m.box('lower_leg', (0, -1.22, 1.24), (.12, .98, .16), 'teal', .032)
    m.box('bracket_neck', (0, -.28, .8), (.16, .04, .16), 'steel', .012)
    m.cylinder('shaft', (0, -2.08, 1.34), .10, .12, 2, 'steel')
    m.box('top_molded_panel', (0, .478, 1), (.38, .005, .35), 'teal', .002)
    for side in (-1, 1):
        m.box('cowling_side_inset', (side * .506, .13, 1), (.006, .218, .34), 'teal', .003)
        for z in (.78, 1, 1.22):
            m.box('cowling_air_vent', (side * .519, .13, z), (.005, .143, .027), 'slate', .004)
        m.box('lower_leg_orange_trim', (side * .122, -1.2, 1.24), (.005, .27, .075), 'coral', .002)
        for z in (.76, 1.24):
            m.cylinder('top_molded_stud', (side * .25, .490, z), .065, .020, 1, 'cream')
        if lod < 2:
            for z in (-.32, .74):
                m.cylinder('bracket_fastener', (side * .34, -.313, z), .043, .014, 1, 'slate')
    m.box('cowling_rear_badge', (0, .15, 1.502), (.19, .105, .006), 'coral', .006)
    m.box('cowling_lower_seam', (0, -.205, 1.002), (.40, .012, .41), 'rubber', .003)
    return m
...
def propeller(lod, part):
    ...
    # and three broad blades preserve the existing blade-tip sweep envelope.
...
def main():
    ...
    for name in PARTS:
        ...
        for lod, binding in enumerate(metadata['lods']):
            model = (engine if name == 'engine' else propeller)(lod, metadata['part'])
