...
import surface_shading as shading
...
class Model:
    ...
    def box(self, name, center, half, color='cream', bevel=.015):
        ...
        bevel = min(bevel, min(half)*.4) if self.lod < 2 else 0
        obj = self.add(geo.object_mesh(name, points,
            [(3,2,1,0),(4,5,6,7),(0,1,5,4),(2,3,7,6),(0,4,7,3),(1,2,6,5)]), color, bevel)
        obj['surface_kind']='rounded_box';obj['surface_center']=center
        obj['surface_half']=half;obj['surface_bevel']=bevel
        return obj
    ...
    def cylinder(self, name, center, radius, depth, axis=1, color='steel'):
        ...
        bevel = min(.006,depth*.2,radius*.2) if self.lod < 2 else 0
        obj = self.add(geo.object_mesh(name, points, faces), color, bevel)
        obj['surface_kind']='rounded_cylinder';obj['surface_center']=center
        obj['surface_axis']=axis;obj['surface_radius']=radius
        obj['surface_depth']=depth;obj['surface_segments']=n;obj['surface_bevel']=bevel
        return obj
    ...
    def ring(self, name, center, radius, thickness, axis=2, color='steel'):
        ...
        obj=self.add(geo.object_mesh(name, points, faces), color, 0)
        obj['surface_kind']='torus';obj['surface_center']=center
        obj['surface_axis']=axis;obj['surface_radius']=radius
        return obj
        ...
        for s in part['sockets']:
            ...
            if s['role'] == 'plug':
                ...
                for obj in bases:
                    ...
                    # Socket cuts introduce interior faces that do not belong
                    # to the outer rounded-box surface.
    ...
    def finish(self, material, surface_shading=shading.FLAT, *, metric_uv=True):
        ...
        for obj in self.objects:
            ...
            if isinstance(material,dict):
                ...
                selected=material
            obj.data.materials.clear(); obj.data.materials.append(selected)
        ...
        if surface_shading!=shading.FLAT:triangulate.keep_custom_normals=True
...
def main():
    ...
    parser.add_argument('--surface-shading',choices=shading.PROFILES,default=shading.FLAT)
    ...
    for name in args.parts:
        ...
        for lod,(tri_limit,vertex_limit,size,threshold) in enumerate(kit.LOD_LIMITS):
            if args.material_profile==geo.materials.TACTILE_METRIC:
                surface=metric.create(geo.params.VERSION_TWO_SPEC['palette_srgb'],maps,lod,geo.png)
            else:
                images=geo.textures(geo.params.VERSION_TWO_SPEC,size,maps,lod,args.material_profile)
                surface=geo.material(images,lod,args.material_profile)
            obj,checks=geometry(name,lod,part).finish(surface,args.surface_shading)
            geo.activate(obj)
            ...
            material_checks=metric.check_export(doc,lod) if args.material_profile==geo.materials.TACTILE_METRIC else None
            vertices=triangles=0
            for mesh in doc['meshes']:
                for p in mesh['primitives']:
                    vertices+=doc['accessors'][p['attributes']['POSITION']]['count']
                    triangles+=doc['accessors'][p['indices']]['count']//3
            material_limit=6 if args.material_profile==geo.materials.TACTILE_METRIC else 1
            if len(doc['meshes'])!=1 or not 1<=len(doc['materials'])<=material_limit or vertices>vertex_limit or triangles>tri_limit:
                raise ValueError(f'{name} LOD{lod} exceeds kit budget: {vertices}/{triangles}')
            metrics.append(dict(lod=lod,vertices=vertices,triangles=triangles,materials=len(doc['materials']),geometry=checks,metric_materials=material_checks,
        ...
        manifest=dict(schema=1,status='offline candidate; not runtime or visual acceptance',
                      ...
                      surface_shading=args.surface_shading,
                      ...
                      recipes={Path(p).name:base.sha256(Path(p)) for p in
                               (__file__,kit.__file__,geo.__file__,geo.params.__file__,geo.materials.__file__,metric.__file__,shading.__file__,base.__file__)},
