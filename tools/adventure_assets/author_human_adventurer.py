...
def materials(lod):
    ...
    for key,color in PALETTE.items():
        ...
        p.inputs['Roughness'].default_value=.42 if key=='skin' else .58
...
def build(lod):
    ...
    def part(name,parent,pivot,draw):
        ...
        obj,check=m.finish(mats,art.shading.FLAT,metric_uv=False)
        def proportion(point):
            x,y,z=point
            if name=='robot_head':return (x*1.14,1.7+(y-1.7)*1.44,z*1.14)
            if name=='robot_torso':return (x*1.08,y*1.18-.365,z)
            if name=='robot_pelvis':return (x*1.08,y*.75+.00225,z)
            return point
    ...
    def pelvis(m):
        m.box('minifigure_hip_block',(0,.786,0),(.190,.087,.125),'moss',.013)
        m.box('printed_leather_belt',(0,.841,-.127),(.181,.023,.002),'leather',0)
        m.box('printed_buckle',(0,.841,-.130),(.022,.018,.001),'brass',0)
    part('robot_pelvis','robot_root',(0,.76,0),pelvis)
    def torso(m):
        # Original classic toy trapezoid silhouette, wider at the waist.
        points=[(sign*x,y,z) for y,x in ((.869,.207),(1.307,.161)) for z in (-.125,.125) for sign in (-1,1)]
        body=m.add(art.geo.object_mesh('trapezoid_toy_torso',points,
            [(0,1,3,2),(4,6,7,5),(0,4,5,1),(2,3,7,6),(0,2,6,4),(1,5,7,3)]),'moss',.010)
        ...
        m.box('brick_daypack',(0,1.10,.166),(.114,.139,.040),'leather',.012)
        m.box('daypack_flap',(0,1.179,.207),(.101,.048,.003),'leather',.003)
        if lod<2:
            for x in (-.104,.104):print_patch('printed_pack_strap',[(x-.011,.955),(x+.011,.955),(x+.011,1.270),(x-.011,1.270)],'leather')
            for y in (.988,1.055):m.box('printed_shirt_button',(0,y,-.127),(.004,.004,.0006),'brass',0)
    part('robot_torso','robot_pelvis',(0,1.025,0),torso)
    def head(m):
        radius=.152
        m.cylinder('yellow_cylindrical_head',(0,1.484,0),radius,.260,1,'skin')
        ...
        n=(20,14,10)[lod]
        for x in (-.054,.054):
            face_patch('printed_friendly_eye',[(x+.013*math.cos(i*2*math.pi/n),1.497+.017*math.sin(i*2*math.pi/n)) for i in range(n)],'dark')
        angles=[-1.03+i*2.06/12 for i in range(13)]
        smile=[((.043+d)*math.sin(a),1.475-(.043+d)*math.cos(a)) for d,series in ((-.0025,angles),(.0025,list(reversed(angles)))) for a in series]
    ...
    for side,sign in (('l',-1),('r',1)):
        ...
        def upper(m,x=x):
            ...
            m.loft('single_molded_sleeve',[(.736,x,-.022,.052,.054),
                ...
                (1.098,x,0,.071,.072),(1.159,x-.008*sign,0,.055,.060)],'cream',n=(16,12,8)[lod])
        part('robot_arm_'+side,'robot_torso',(x,1.135,0),upper)
        def lower(m,x=x):
            # The retained forearm node carries only a small covered wrist.
            m.cylinder('yellow_toy_wrist',(x,.720,-.023),.032,.048,1,'skin')
        part('robot_forearm_'+side,'robot_arm_'+side,(x,.82,0),lower)
        def hand(m,x=x):
            n=(24,18,12)[lod];cross=(8,6,5)[lod];points=[]
            for i in range(n+1):
                a=math.radians(-45+270*i/n)
                for j in range(cross):
                    b=j*2*math.pi/cross;r=.060+.014*math.cos(b)
                    points.append((x+r*math.cos(a),.653+r*math.sin(a),-.026+.014*math.sin(b)))
        ...
        part('robot_hand_'+side,'robot_forearm_'+side,(x,.70,0),hand)
    for side,x in (('l',-.108),('r',.108)):
        ...
        part('robot_thigh_'+side,'robot_pelvis',(x,.55,0),thigh)
        def shin(m,x=x):
    ...
    for name,parent,pivot in (
        ...
        ('robot_helm_r_anchor','robot_hand_r',(.215,.653,-.026))):nodes[name]=compatible.empty(name,nodes[parent],pivot)
    return nodes,checks
...
def animation_pose(clip,fraction):
    ...
    if clip=='idle':angle('torso',.005*math.sin(theta),0,0)
    elif clip=='walk':
        swing=.32*math.cos(theta)
        offset.y=max(0,.55*(math.cos(swing)-1)+.171*abs(math.sin(swing)))+.0008
        for side,sign in (('l',1),('r',-1)):
...
def main():
    ...
    for lod in range(3):
        ...
        assert vertices<=((10500,7500,4500)[lod]) and triangles<=((11000,7500,4500)[lod]),(vertices,triangles)
        blend=source/f'human-lod-{lod}.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend),compress=True)
        records.append(dict(lod=lod,asset_id=f'voxys-adventure-human-r01-lod{lod}',glb=path.name,glb_sha256=sha(path),
    ...
    helpers=[Path(__file__),Path(compatible.__file__),Path(art.__file__),Path(art.geo.__file__),Path(art.base.__file__),Path(art.metric.__file__),Path(art.shading.__file__)]
    provenance=dict(schema=1,asset='original-warm-brick-minifigure',author='Voxys original procedural artwork',
        ...
        reproduction='/snap/blender/current/blender --background --factory-startup -noaudio --threads 2 --python-exit-code 1 --python tools/adventure_assets/author_human_adventurer.py -- --output-dir <new-directory>',
