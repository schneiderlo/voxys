...
void updateScopeString() {
    currentScopeStr.clear();
    for (size_t i = 0; i < scopeStack.size(); ++i) {
        if (i > 0) currentScopeStr += "::";
        currentScopeStr += scopeStack[i];
    }
}
...
void outputMessage(Level level, std::string_view message) {
    ...
    if (s.timestampEnabled && !currentScopeStr.empty()) {
        fullMsg = std::format("[{}] [{}] [{}] {}", 
                              levelToString(level), 
                              formatTimestamp(), 
                              currentScopeStr,
