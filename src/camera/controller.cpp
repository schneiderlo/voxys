...
void FreeFlyController::processMovement(float deltaTime, const Input& input) {
    ...
    // Forward/Backward (along camera forward direction). Arrow keys are provided
    // as layout-independent alternates: WASD is reported positionally (US layout),
    // so the letters shift on AZERTY/QWERTZ keyboards while the arrows do not.
