...
bool MipGeneratorPipeline::generateMipChain(WGPUDevice device, WGPUQueue queue,
    ...
    if (!device || !queue || !texture || mipLevelCount < 2) {
        LOG_ERROR("MipGeneratorPipeline: Invalid device, queue, texture, or mip level count");
        return false;
    }
    ...
    std::vector<WGPUBuffer> paramsBuffersToRelease;

    const auto releaseRecordedResources = [&]() {
        for (auto bg : bindGroupsToRelease) wgpuBindGroupRelease(bg);
        for (auto view : viewsToRelease) wgpuTextureViewRelease(view);
        for (auto buffer : paramsBuffersToRelease) {
    ...
    for (uint32_t level = 1; level < mipLevelCount; level++) {
        ...
        // Queue writes are applied before the batched command buffer executes.
        // Each pass therefore needs immutable parameter storage of its own;
        // rewriting paramsBuffer_ here would make every pass see the final level.
        MipParams params{
            .srcWidth = srcWidth,
            .srcHeight = srcHeight,
            .dstWidth = dstWidth,
        ...
        const auto paramsDesc = gpu::BufferDesc::uniform(sizeof(MipParams), "mip_level_params");
        WGPUBuffer levelParamsBuffer = gpu::createBufferWithData(
            device, queue, paramsDesc, std::span<const MipParams>(&params, 1));
        if (!levelParamsBuffer) {
            LOG_ERROR("MipGeneratorPipeline: Failed to create params buffer for level {}", level);
            releaseRecordedResources();
            wgpuCommandEncoderRelease(encoder);
            return false;
        }
        paramsBuffersToRelease.push_back(levelParamsBuffer);
        ...
        std::array<gpu::BindGroupEntry, 3> entries = {
            ...
            gpu::BindGroupEntry(2).buffer(levelParamsBuffer, 0, sizeof(MipParams))
        };
