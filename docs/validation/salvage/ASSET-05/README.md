...
**Installed candidate:** [r09 geometry/proxy corrections and all-detail runtime checks](r09-correction.md).
The [r09 close-contact checkpoint](r09-contacts/README.md) completes root's
24 paired close views at all three detail levels. ASSET-05 remains open for
independent review; material findings continue in ASSET-06.
The preceding README is preserved in `r08-inspection-checkpoint.md`.

**Earlier follow-up:** [r08 controls, contact captures, motion and remaining defects](inspection.md).
The original text below describes the earlier static checkpoint. Its exact
pre-follow-up bytes are preserved in `static-kit-checkpoint.md`; `summary.json`
is that historical checkpoint, and `inspection-summary.json` binds the follow-up.
