...
def owned_design(payload):
    ...
    assert len(builds) == 1 and builds[0]['parts'] == 13
    assert all(int.from_bytes(p[89:91], 'little') == 10000 for p in all_parts)
    assert sum(p[105] == 1 for p in all_parts) == 11  # Retained starter loans.
...
def main():
    ...
    def pulse(button):
        ...
        pad.button(button, True); time.sleep(.09); pad.button(button, False)
        state = fresh()
    ...
    try:
        ...
        assert len(starter_parts) == 11 and library()['rows'] == []
        added = []; total_cost = 0
        for number in (1, 2):
            ...
            ghost = wait(lambda s: s['workshop']['parts'] == 11 + number and s['workshop']['changed'] and s['workshop']['valid'], 'valid snapped one-shot brick')
            ...
            if number == 1:
                ...
                try:
                    ...
                    assert held['workshop']['changed'] and held['workshop']['placedParts'] == 11
            ...
            sample(f'controller-brick-{number}-kept', lambda s: not s['workshop']['changed'] and s['workshop']['placedParts'] == 11 + number)
        ...
        assert redone['workshop']['parts'] == undone['workshop']['parts'] == 13
        ...
        wait(lambda s: menu(s)['page'] == 'main' and s['workshop']['parts'] == 13 and not s['workshop']['changed'], 'restored named design loaded explicitly')
        sample('named-update-backup-restore-load')
        exported, expected_blueprint, final_parts = export('TRANSFER')
        assert len(final_parts) == 13
        ...
        launched = sample('two-brick-owned-launch', lambda s: not s['workshop']['open'] and not s['workshop']['pending'] and s['boat']['parts'] == 13)
        ...
        wait(lambda s: menu(s)['page'] == 'main' and not s['workshop']['changed'] and s['workshop']['parts'] == 13,
