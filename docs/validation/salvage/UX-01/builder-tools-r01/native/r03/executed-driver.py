...
class MappingProbe:
    """Read the installed GLFW standard mapping without creating any window.

    This second read-only client does not send input, change mappings or touch
    focus. Its sample proves the OS descriptor's button mapping, not that the
    game consumed an edge; the journey's state assertions prove that separately.
    """
    class State(C.Structure):
        _fields_ = [('buttons', C.c_ubyte * 15), ('axes', C.c_float * 6)]

    def __init__(self):
        self.glfw = C.CDLL('libglfw.so.3')
        for name, args_, result in (
            ('glfwInit', [], C.c_int), ('glfwTerminate', [], None),
            ('glfwGetJoystickName', [C.c_int], C.c_char_p),
            ('glfwGetJoystickGUID', [C.c_int], C.c_char_p),
            ('glfwGetGamepadName', [C.c_int], C.c_char_p),
            ('glfwGetGamepadState', [C.c_int, C.POINTER(self.State)], C.c_int)):
            fn = getattr(self.glfw, name); fn.argtypes = args_; fn.restype = result
        assert self.glfw.glfwInit(), 'Read-only GLFW mapping probe could not initialize'

    def read(self):
        for device in range(16):
            name = self.glfw.glfwGetJoystickName(device)
            if name != b'Voxys acceptance controller': continue
            state = self.State()
            assert self.glfw.glfwGetGamepadState(device, C.byref(state)), 'Acceptance controller has no standard GLFW mapping'
            return {'device': device, 'name': name.decode(),
                    'guid': (self.glfw.glfwGetJoystickGUID(device) or b'').decode(),
                    'mappedName': (self.glfw.glfwGetGamepadName(device) or b'').decode(),
                    'buttons': list(state.buttons), 'axes': list(state.axes)}
        return None

    def close(self): self.glfw.glfwTerminate()
...
def main():
    ...
    controls = pad = probe = child = stream = window = None
    ...
    def pulse(button):
        ...
        pad.button(button, True); time.sleep(.10)
        try:
            mapping = probe.read()
            assert mapping, 'Read-only mapping probe lost the acceptance controller'
            standard = {'a': 0, 'b': 1, 'x': 2, 'y': 3, 'lb': 4, 'rb': 5,
                        'view': 6, 'menu': 7, 'home': 8, 'ls': 9, 'rs': 10}
            assert mapping['buttons'][standard[button]] == 1, (button, mapping)
            report.setdefault('mappingObserved', {}).setdefault(button, mapping)
            time.sleep(.10)
    ...
    try:
        ...
        pad = VirtualGamepad(); probe = MappingProbe(); start(); initial = sample('fresh-owned-world')
