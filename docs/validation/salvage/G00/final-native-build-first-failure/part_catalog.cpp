...
[[nodiscard]] CatalogError validateModule(const PartDefinition& part) {
    ...
    return std::visit([&part](const auto& module) -> CatalogError {
        ...
        if constexpr (std::is_same_v<T, FlotationModule>) {
            ...
            socketError = requireSocket(part, module.line, SocketFamily::TowLine, SocketRole::Plug);
            if (socketError == CatalogError::None) {
                valid = valid && module.maximumForceNewtons <= findSocket(part, module.line)->strength.tensionNewtons;
            }
