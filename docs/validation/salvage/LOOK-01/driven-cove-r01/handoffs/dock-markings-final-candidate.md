...
4. `FixtureGPU.DockMarkingsUseExactOwnerChargeSharedDrawTicketAndRetirement`: one-byte-over-budget CPU admission rejects before creating a candidate; actual owner charge increments once; missing geometry/root refuses before ticket; two draws share owner submission; opt-in RGBA8 + Depth32 numeric readback draws the real admitted dock under one fixed top-down camera, requires changed opaque teal/orange pixels with nearer depth and unchanged remaining pixels, and requires a hidden submission to restore baseline color/depth exactly; hidden encoding/discard cannot advance submitted counts; hidden submission reports zero; Leave drains marking storage.
...
The final GPU case additionally performs three numeric readbacks (baseline, visible, hidden) within this same owner/lifetime case and one camera. It writes no files or images. Copy-source target usage is opt-in for that case only; all other fixture tests keep their previous target flags. The new map callback owns only a shared CPU completion record, and target copies share the actual scene queue submission/ticket. None of these GPU assertions has been run by this agent.
