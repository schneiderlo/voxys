        ...
        return {ready,presentationReady,frame:Number(engine?._voxy_get_frame_count?.()||0),state,
            ...
            semantics:{present:!!host,mode:host?.dataset.mode,role:host?.getAttribute('role'),
                peers:[...(host?.querySelectorAll('button')||[])].map(b=>({label:b.getAttribute('aria-label'),disabled:b.disabled,
                    ...
                    shortcut:b.getAttribute('aria-keyshortcuts'),pressed:b.getAttribute('aria-pressed'),
...
function verify(snapshot){
    ...
    assert.equal(state.quickSlots.length,6);
    for(const control of hud.controls.filter(c=>c.action===39)){
        const peer=semantics.peers.find(p=>p.action===39&&p.value===control.value);
        assert.equal(control.shortcutKey,48+control.value,'Visible number must match native shortcut');
        assert.equal(peer.shortcut,control.enabled===false?null:String(control.value));
        assert.equal(peer.pressed,String(control.value===state.quickSlot));
    }
    if(state.navigation.visible){
        assert(state.navigation.mapRevision>0,'Navigation must have a real terrain map');
        assert(Number.isFinite(state.navigation.cameraBearingDegrees));
        assert(Number.isFinite(state.navigation.playerBearingDegrees));
        assert.equal(state.navigation.playerUv.length,2);
        assert(state.navigation.playerUv.every(Number.isFinite));
        assert(state.navigation.placedPieces>=0);
    }
}
async function clickControl(label,match,expectedMode,check=()=>true){
    ...
    report.actions.push({label,control,position,before:{frame:before.frame,mode:before.state.mode,token:before.state.menuToken},
        after:{frame:after.frame,mode:after.state.mode,token:after.state.menuToken,piece:after.state.piece,paint:after.state.paint,quickSlot:after.state.quickSlot,parts:after.state.parts}});
...
try{
    ...
    await clickControl('White brick preset',c=>c.action===39&&c.value===2,'build',state=>state.quickSlot===2&&state.piece===10&&state.paint===0xf3f2eb);
    await clickControl('Red brick preset',c=>c.action===39&&c.value===1,'build',state=>state.quickSlot===1&&state.piece===9&&state.paint===0xe53b33);
    ...
    await clickControl('Choose blue',c=>c.action===10&&c.value===0x2d91cc,'build',state=>state.paint===0x2d91cc&&state.quickSlots[state.quickSlot-1].paint===0x2d91cc);
    ...
    assert(final.state.hud.controls.every(c=>c.action!==39),'Walking still exposes build-piece controls');
