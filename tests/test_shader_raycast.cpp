...
TEST_F(RaycastShaderTest, HasUniformBindings) {
    ...
    EXPECT_NE(shaderSource_.find("@group(0) @binding(4)"), std::string::npos)
        << "Shader missing outMaterial binding";
}
...
TEST_F(RaycastShaderTest, HasWaterMaterialOutput) {
    ASSERT_FALSE(shaderSource_.empty());
    EXPECT_NE(shaderSource_.find("MATERIAL_WATER"), std::string::npos)
        << "Shader missing water material id";
    EXPECT_NE(shaderSource_.find("camera.waterParams"), std::string::npos)
        << "Shader missing water uniform usage";
    EXPECT_NE(shaderSource_.find("tWater"), std::string::npos)
        << "Shader missing water plane intersection";
    EXPECT_NE(shaderSource_.find("MIN_WATER_DEPTH"), std::string::npos)
        << "Shader missing minimum water depth gate";
    EXPECT_NE(shaderSource_.find("nearbyShoreInfluence"), std::string::npos)
...
TEST_F(RaycastShaderTest, RayGenerationUsesProjectionScaleAndViewRotation) {
    ...
    EXPECT_NE(shaderSource_.find("camera.invProjParams.xy"), std::string::npos)
        << "Ray generation should use inverse projection scale";
    EXPECT_NE(shaderSource_.find("camera.invView *"), std::string::npos)
        << "Ray generation should rotate view rays into world space";
}
...
TEST_F(RaycastShaderTest, StartsAtCoarseMipLevel) {
    ASSERT_FALSE(shaderSource_.empty());
    EXPECT_NE(shaderSource_.find("fn maxTraversalMip()"), std::string::npos)
        << "Shader should calculate a valid coarse mip for the bound texture";
    EXPECT_NE(shaderSource_.find("textureNumLevels(heightTex)"), std::string::npos)
        << "Shader should clamp traversal to the available mip count";
    EXPECT_NE(shaderSource_.find("mipLevel : u32 = maxMipLevel"), std::string::npos)
        << "Shader should start at the coarsest valid traversal mip";
}
