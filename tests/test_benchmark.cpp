...
TEST(BrowserJourneyBenchmarkTest, ThrowsIntoPrebuiltCubeTriangle) {
    BrowserJourneyBenchmark journey;
    ASSERT_TRUE(journey.start(
        BrowserJourneyConfig{
            .targetBodies = 128,
    ...
    EXPECT_EQ(journey.advance(11, 20'022), 128u);
    ASSERT_TRUE(journey.reportVolley(11, 128, 128));
    journey.recordFrame({
        .frame = 6,
        .physicsTick = 11,
        .wallMilliseconds = 12.0f,
        .cpuMilliseconds = 1.0f,
        .residentBodies = 20'150,
        .activeBodies = 128,
        .submittedPrimitives = 20'150,
    });
    EXPECT_EQ(journey.advance(13, 20'150), 0u);
    journey.recordFrame({
        .frame = 7,
        .physicsTick = 13,
        .wallMilliseconds = 10.0f,
        ...
        .residentBodies = 20'150,
        .activeBodies = 512,
        .candidatePairs = 1'024,
        .contacts = 256,
        .submittedPrimitives = 20'150,
    });
    EXPECT_EQ(journey.advance(14, 20'150), 0u);
    journey.recordFrame({
        .frame = 8,
        .physicsTick = 14,
        .wallMilliseconds = 9.0f,
    ...
    EXPECT_TRUE(journey.passed());
    EXPECT_EQ(journey.spawnedBodies(), 128u);
    EXPECT_EQ(journey.volleyCount(), 1u);
    const std::string json = journey.resultJson();
    EXPECT_NE(json.find("\"baseline_bodies\":20022"),
              std::string::npos);
    EXPECT_NE(json.find("\"expected_final_bodies\":20150"),
              std::string::npos);
}
