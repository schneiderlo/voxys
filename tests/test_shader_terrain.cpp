...
TEST_F(TerrainShaderTest, ShadowTraversalClampsToAvailableMipLevels) {
    ASSERT_FALSE(shaderSource_.empty());
    EXPECT_NE(shaderSource_.find("fn maxTraversalMip()"), std::string::npos)
        << "Triangle shadows must calculate a valid maximum mip";
    EXPECT_NE(shaderSource_.find("textureNumLevels(heightTex)"), std::string::npos)
        << "Triangle shadows must respect the bound texture's mip count";
    EXPECT_NE(shaderSource_.find("mipLevel < maxMipLevel"), std::string::npos)
        << "Triangle shadows must not ascend beyond the valid maximum mip";
    EXPECT_EQ(shaderSource_.find("mipLevel < 7u"), std::string::npos)
        << "Triangle shadows must not assume that mip level 7 exists";
}
