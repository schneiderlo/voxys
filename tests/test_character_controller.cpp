...
TEST(CharacterConfigTest, TerrainExtentCountsIntervalsBetweenSamples) {
    EXPECT_FLOAT_EQ(terrainSampleExtent(64, 1.0f), 63.0f);
    EXPECT_FLOAT_EQ(terrainSampleExtent(4, 2.5f), 7.5f);
    EXPECT_FLOAT_EQ(terrainSampleExtent(1, 2.5f), 2.5f);
}

TEST(CharacterConfigTest, RendererEdgeCoordinatesReachHeightmapEdges) {
    std::vector<uint16_t> heights = {0, 65535};
    auto heightmap = terrain::Heightmap::createFromData(std::move(heights), 2, 1);

    CharacterConfig config;
    config.heightScale = 10.0f;
    config.cellScale = 2.0f;
    config.terrainWidth = terrainSampleExtent(heightmap.getWidth(), config.cellScale);
    config.terrainHeight = terrainSampleExtent(heightmap.getHeight(), config.cellScale);

    Camera camera;
    CharacterController controller(camera, &heightmap, config);

    EXPECT_FLOAT_EQ(controller.sampleTerrainHeight(-1.0f, 0.0f), -10.0f);
    EXPECT_FLOAT_EQ(controller.sampleTerrainHeight(1.0f, 0.0f), 10.0f);
}
