// Compare staged releases using the same Chrome executable, viewport and GPU.
// Example: node measure_startup.mjs --chrome=PATH --site=DIR --output=DIR
// Each invocation starts a fresh Chrome profile, then reloads in that profile.
...
try{
    ...
    const url=`http://127.0.0.1:${server.address().port}/?new=1&telemetry=0`;
    report.url=url;
    for(const cache of [args.profile?'retained-profile':'fresh-profile','reload']){
        ...
        while(Date.now()-started<600000){
            ...
            const result=await call('Runtime.evaluate',{returnByValue:true,expression:`(() => {
                ...
                return {build:window.voxyBuildId,ready,telemetry,device:window.voxyDeviceProfile,
                    ...
                    pipelines:globalThis.startupPipelineTimings||[]};
            })()`},Math.max(1000,600000-(Date.now()-started)));
            if(result.exceptionDetails)throw Error(JSON.stringify(result.exceptionDetails));
            current.sample=result.result.value;current.elapsed_ms=Date.now()-started;
            const s=current.sample;
            if(s.ready&&current.initialized_ms===undefined)current.initialized_ms=current.elapsed_ms;
            if(s.lost||s.errors?.length||s.pageError||current.exceptions.length)throw Error('Application error: '+JSON.stringify({lost:s.lost,errors:s.errors,pageError:s.pageError}));
            if(s.ready&&!s.loading&&s.telemetry?.frame?.count>=30&&s.telemetry.render_gpu?.available){
