...
const began=Date.now(),report={status:'running',startedUtc:new Date().toISOString(),profile,origin:url.origin,
    scope:'Ordinary-input functional journey. Screenshot requires independent visual review.',
...
try{
    ...
    await save('before-default-entry',false);
    await page.goto(new URL('index.html',url).href,{waitUntil:'domcontentloaded',timeout:30000});
    await wait(s=>s.world===saved.world&&s.saveStatus==='Saved adventure loaded','bare default entry restores adventure',90000);
    assert.deepEqual(inventory(last),expected,'Default entry kept the existing home');
    await record('bare-default-entry-restored-adventure');await releaseWorld();
    assert(!last.build&&!last.menu);const beforeClick=inventory(last),box=await page.locator('#voxy-canvas').boundingBox();
    await page.locator('#voxy-canvas').click({position:{x:box.width*.65,y:box.height*.8}});await fresh();
    assert.equal(await page.evaluate(()=>document.pointerLockElement),null,'Adventure canvas click must not lock the pointer');
    assert.equal(last.aimInput.mouseCaptured,false);assert.deepEqual(inventory(last),beforeClick);
    await record('ordinary-canvas-click-kept-pointer-free');
    await walk([ox-.58,oz+1.25],.08);await walk([ox-1,oz+1]);await walk([ox-1,oz+5]);await walk([ox+7,oz+5]);
