...
def main():
    ...
    try:
        ...
        if args.phase in ('restore','all'):
            ...
            # A restore-only continuation already opened this exact saved world.
            # Reuse that process instead of performing a redundant second boot.
            if args.phase!='restore' or not prior:
                stop();start(world)
            restored=record('robot-camera-world-restored-paused')
