...
def main():
    ...
    parser.add_argument('--continue-from', type=Path, help='Retained r04 output: reuse its durable library, not its unsaved world')
    args = parser.parse_args()
    args.binary = args.binary.resolve(strict=True)
    args.output = args.output.resolve(); args.storage_root = args.storage_root.resolve()
    args.output.mkdir(parents=True, exist_ok=False)
    if args.continue_from:
        args.continue_from = args.continue_from.resolve(strict=True)
        assert args.storage_root.is_dir()
        assert not any((p / 'current').exists() for p in args.storage_root.iterdir() if re.fullmatch('[0-9a-f]{32}', p.name)), 'Continuation expects no prior world checkpoint'
    else:
    ...
    try:
        ...
        if args.continue_from:
            prior_path = args.continue_from / 'summary.json'
            prior = json.loads(prior_path.read_text())
            prior_state = prior['lastState']; prior_library = prior_state['nativeMenu']['library']
            assert prior['status'] == 'failed' and prior_state['nativeHud']['truncated']
            assert prior_library['generation'] == '5' and Path(prior_library['root']) == args.storage_root / 'Designs'
            assert initial['world'] != prior_state['world']
            def retained_export(name):
                entry = next(e for e in prior['exports'] if e['name'] == name)
                evidence = args.continue_from / entry['evidence']; source = Path(entry['source'])
                assert source.parent == args.storage_root / 'Designs' / 'Exports'
                assert source.read_bytes() == evidence.read_bytes()
                assert hashlib.sha256(evidence.read_bytes()).hexdigest() == entry['fileSha256']
                value, raw, parts = blueprint_file(evidence)
                assert value['name'] == name and hashlib.sha256(raw).hexdigest() == entry['blueprintSha256']
                return source, raw, parts
            exported, expected_blueprint, final_parts = retained_export('TRANSFER')
            _, _, starter_parts = retained_export('HULL')
            assert len(starter_parts) == 10 and len(final_parts) == 12
            remaining = [p[4:] for p in final_parts]
            for part in starter_parts: remaining.remove(part[4:])
            assert len(remaining) == 2 and all(list(p[41:45]) == TEAL for p in remaining)
            recorded_bricks = [stage['state']['workshop'] for stage in prior['stages'] if stage['name'] in ('controller-brick-1-kept', 'controller-brick-2-kept')]
            assert len(recorded_bricks) == 2
            assert sorted((list(struct.unpack_from('<iii', p, 28)), p[40]) for p in remaining) == sorted((p['placement'], p['rotation']) for p in recorded_bricks)
            report['continuation'] = {'source': str(args.continue_from),
                'sourceSummarySha256': hashlib.sha256(prior_path.read_bytes()).hexdigest(),
                'sourceWorld': prior_state['world'], 'freshWorld': initial['world'],
                'meaning': 'The earlier world was never checkpointed. This fresh world reuses only its durable named library and exact exchange files.',
                'sourceLibraryGeneration': '5', 'sourceBlueprintSha256': hashlib.sha256(expected_blueprint).hexdigest(),
                'sourceTransferFileSha256': hashlib.sha256(exported.read_bytes()).hexdigest()}
            library_page()
            assert library()['generation'] == '5' and library()['rows'] == prior_library['rows']
            choose('TUG'); choose('Load into workshop')
            loaded = sample('durable-r04-design-loaded-in-fresh-world', lambda s: menu(s)['page'] == 'main' and s['workshop']['parts'] == 12 and not s['workshop']['changed'])
            assert loaded['boat']['paidPartIds'] == [] and loaded['boat']['parts'] == 11
            expected_mass = loaded['workshop']['massKg']; total_cost = int(loaded['workshop']['charge']); expected_stock = 48 - total_cost
            assert expected_mass == prior_state['workshop']['massKg'] and total_cost == int(prior_state['workshop']['charge'])
            _, loaded_bytes, _ = export('LOADED')
            assert loaded_bytes == expected_blueprint and library()['generation'] == '5'
        else:
        ...
        valid = imports / 'transfer.voxy-design.json'
        if not args.continue_from:
            shutil.copyfile(exported, valid)
            ...
            assert valid.read_bytes() == exported.read_bytes()
            try: blueprint_file(imports / 'corrupt.voxy-design.json')
            except (AssertionError, ValueError): pass
            else: raise AssertionError('The retained corrupted exchange copy must still be invalid')
