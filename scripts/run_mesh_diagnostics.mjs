...
const [packageArg,outputArg,kind='mesh']=process.argv.slice(2);
assert(['mesh','environment','mesh-environment','mesh-opaque'].includes(kind),'Unknown diagnostic kind');
assert(packageArg&&outputArg,'Usage: node scripts/run_mesh_diagnostics.mjs BUILD_DIRECTORY NEW_OUTPUT_DIRECTORY');
const root=path.resolve(packageArg),output=path.resolve(outputArg);
await mkdir(output,{recursive:false});
const profile=await mkdtemp(path.join(tmpdir(),'voxys-mesh-browser-'));
const sha=bytes=>createHash('sha256').update(bytes).digest('hex');
const delay=ms=>new Promise(resolve=>setTimeout(resolve,ms));
const report={passed:false,kind:`Shared C++ ${kind} correctness tests on hardware browser WebGPU`,
    runner_sha256:sha(await readFile(fileURLToPath(import.meta.url))),package:root,artifacts:{}};
for(const name of ['index.html','mesh_diagnostics.js','mesh_diagnostics.wasm']){
...
const flags=['--no-sandbox','--no-first-run','--no-default-browser-check','--disable-background-networking',
    '--enable-unsafe-webgpu','--disable-gpu-watchdog','--disable-background-timer-throttling',
    '--disable-renderer-backgrounding','--ozone-platform=wayland','--enable-features=Vulkan',
...
try{
    ...
    assert(result,'C++ rendering tests timed out');
    await writeFile(path.join(output,'tests.log'),result.output.join('\n')+'\n');
    report.result={...result,captures:[]};
    for(const capture of result.captures){
        const bytes=Buffer.from(capture.pixels),png=Buffer.from(capture.png.split(',')[1],'base64');
        await writeFile(path.join(output,capture.name),bytes);
        await writeFile(path.join(output,capture.name.replace('.rgba','.png')),png);
        const record={name:capture.name,metadata:capture.metadata,sha256:sha(bytes),png_sha256:sha(png)};
        for(const field of ['float32','requests'])if(capture[field]){
            const raw=Buffer.from(capture[field]),suffix=field==='float32'?'.f32':'.requests.f32';
            await writeFile(path.join(output,capture.name.replace('.rgba',suffix)),raw);
            record[field+'_sha256']=sha(raw);
        }
        report.result.captures.push(record);
    }
    if(kind!=='mesh-opaque'){
    const screenshot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
    await writeFile(path.join(output,'material-sheet.png'),Buffer.from(screenshot.data,'base64'));
    }
    // One intentional malformed CPU asset must log rejection; other errors fail.
    const consoleErrors=consoleMessages.filter(m=>m.type==='error').map(m=>m.args.map(a=>a.value??a.description??'').join(' '));
    const expectedRejection=/^\[ERROR\] \[\d{2}:\d{2}:\d{2}\.\d{3}\] MeshPath: (?:failed to upload VMESH data|blended materials require a later transparency pass)$/;
    report.expected_rejection_logs=consoleErrors.filter(text=>expectedRejection.test(text));
    report.unexpected_console_errors=consoleErrors.filter(text=>!expectedRejection.test(text));
    assert.equal(report.expected_rejection_logs.length,(kind==='mesh'||kind==='mesh-opaque')?1:0,'Missing/extra malformed-asset rejection');
