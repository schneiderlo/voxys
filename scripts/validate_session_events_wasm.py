#!/usr/bin/env python3
"""Compile bounded session-event tests with shipping JS EH/Asyncify; run in Node."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    sources = ["src/game/construction/construction_types.cpp", "src/game/expedition/session_events.cpp",
               "tests/test_session_events.cpp"]
    headers = ["src/game/construction/construction_types.hpp", "src/game/construction/part_catalog.hpp",
               "src/game/construction/build_model.hpp", "src/game/expedition/session_transactions.hpp",
               "src/game/expedition/session_events.hpp"]
    ...
    try:
        ...
        for source in [gtest / "src/gtest-all.cc", gtest / "src/gtest_main.cc"]:
            ...
            objects.append(str(obj))
        binary = output / "session_events_tests.js"
        ...
        if not count or int(count.group(1)) != 15 or "[  SKIPPED ]" in log or "DISABLED TEST" in log:
            raise RuntimeError("expected all 15 event cases, zero skips/disabled")
