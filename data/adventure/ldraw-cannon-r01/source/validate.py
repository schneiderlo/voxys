...
assert len(external) == manifest['library']['external_dependency_files']
assert len(visited) - len(external) == manifest['library']['embedded_reachable_files']
print(f'PASS: source hashes; {len(external)} external files; {len(visited)-len(external)} embedded files; zero missing dependencies.')
