...
def textures(spec, size, maps, lod, profile=materials.LEGACY):
    if profile not in materials.PROFILES:
        raise ValueError('unknown material profile')
...
def main():
    ...
    manifest={'schema':1,'status':'candidate; offline authoring only, not engine acceptance',
        ...
        'maps':('Flat sRGB palette and linear MR at every LOD; no normal/grain until metric UV detail is authored'
