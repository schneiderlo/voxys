...
def classify_face(face, obj, spec):
...
def unwrap(obj, spec):
    ...
    for face in obj.data.polygons:
        region = REGIONS[classify_face(face, obj, spec)]
...
def textures(spec, size, maps, lod):
...
def material(images, lod):
    ...
    shader.inputs['Metallic'].default_value = 0 if lod == 2 else 1
    shader.inputs['Roughness'].default_value = .49 if lod == 2 else 1
...
def export(obj, source, lod):
    ...
    if len(document.get('meshes',[]))!=1 or len(document.get('materials',[]))!=1:
        raise ValueError('expected one mesh/material per LOD')
    ...
    return {'file':path.name,'sha256':params.sha256(path),'bytes':len(data),'triangles':triangles,
            'vertices':vertices,'materials':1,'meshes':1,'export_settings':settings}
...
def main():
    ...
    for lod,(_,_,size,_) in enumerate(params.LOD_LIMITS):
        ...
        obj.data.materials.append(material(textures(spec,size,maps,lod),lod))
        for face in obj.data.polygons:
            face.material_index=0
        exported=export(obj,source,lod)
        ...
        exported['base_color_texel_density']=texture_density(obj,size)
    ...
    manifest={'schema':1,'status':'candidate; offline authoring only, not engine acceptance',
        ...
        'export_to_canonical_rotation':12,'uv_quantization':'1/65536 before export tangent generation',
        'maps':'Original deterministic procedural sRGB color and separate linear normal/MR; no baked lighting, no external imagery',
