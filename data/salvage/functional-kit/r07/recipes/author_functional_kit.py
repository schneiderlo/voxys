...
def geometry(name, lod, part):
    ...
    if name in ('beam','plate'):
        ...
        m.mounts(part,[mount])
        m.box('lower_leg',(0,-.82,1.24),(.12,.58,.16),'teal',.035)
        m.box('bracket_neck',(0,-.28,.8),(.16,.04,.16),'steel')
        m.cylinder('shaft',(0,-1.28,1.34),.10,.12,2)
        ...
        m.mounts(part,[mount])
        m.box('console_column',(0,-.13,-.12),(.19,.22,.24),'cream',.035)
        ...
        m.cylinder('cable_drum',(0,.12,0),.28,.56,0,'slate')
        for x in (-.30,.30):
            m.cylinder('drum_flange',(x,.12,0),.36,.06,0,'steel')
        if lod < 2:
            for i in range(7):
                m.ring('cable_wrap',(-.24+i*.08,.12,0),.28,.018,0,'slate')
...
def main():
    ...
    for name in args.parts:
        ...
        for lod,(tri_limit,vertex_limit,size,threshold) in enumerate(kit.LOD_LIMITS):
            images=geo.textures(geo.params.VERSION_TWO_SPEC,size,maps,lod)
            obj,checks=geometry(name,lod,part).finish(geo.material(images,lod))
            geo.activate(obj)
        ...
        manifest=dict(schema=1,status='offline candidate; not runtime or visual acceptance',
                      ...
                      recipes={Path(p).name:base.sha256(Path(p)) for p in
                               (__file__,kit.__file__,geo.__file__,geo.params.__file__,base.__file__)},
